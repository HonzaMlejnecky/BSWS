...
export default function DatabasesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      {databases.length === 0 ? (
        <EmptyState title="No databases yet" buttonText="Create Database" onAction={() => setShowCreate(true)} />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {databases.map((db) => <DatabaseCard key={db.id} db={db} onDelete={deleteDatabase} />)}
        </div>
      )}

      <CreateDatabaseModal open={showCreate} onClose={() => setShowCreate(false)} onCreate={addDatabase} />
    </div>
  );
}
