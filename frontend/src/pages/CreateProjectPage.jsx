...
export default function CreateProjectPage() {
  ...
  useEffect(() => {
    subscriptionsApi.getMine().then((subs) => {
      ...
      if (!active) navigate('/plan', { replace: true });
    ...
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="bg-white rounded-2xl shadow-md p-6 border border-gray-100 max-w-2xl">
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <input className="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200" placeholder="Doména nebo hostname (např. demo-1.local)" value={domain} onChange={(e) => setDomain(e.target.value)} required />
